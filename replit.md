...
- `src/lib/default-data.ts` — seeded sample data + exported constants (`RESEARCH_TYPES`, `SOURCE_TYPES`, `GPS_STEPS`). Also seeds `announcements` and the browser-only `admin` object (`{ unlocked, passcode }`, default passcode `landon-admin`).
- `src/lib/announcements.ts` — announcement helpers shared by the Race Control page + dashboard banner: `ANNOUNCEMENT_LEVELS` (Info/Important/Critical), `ANNOUNCEMENT_CATEGORIES`, `levelAccent(level)` (Critical→red, Important→sky, Info→slate — on-brand only), `sortAnnouncements()` (pinned first, then newest date).
